...
def marriage():
    ...
    for i in range(min(len(waiting_couple_list_male), len(waiting_couple_list_female))-1):
        ...
        #print([waiting_couple_list_female[i], waiting_couple_list_male[i], 0])
new_pop = 0
def get_children(new_pop):
    for a in couple:
        female_age = a[0][1]
        male_age = a[1][1]
...
for i in range(sim_ppl):
    human.append([random.randint(0,1), random.randint(0, 20), 0, 0])
sim_years = 100
#按年模拟
temp_to_csv_1 = []
dead_pop_old = 0
for i in range(sim_years):
    ...
    for a in human:
        ...
        if a[-1] == 1:
            dead_pop += 1
    population = female_pop + male_pop

    temp_to_csv_1.append([population, female_pop, male_pop, dead_pop - dead_pop_old, new_pop])
    dead_pop_old = dead_pop
...
for a in human:
    if a[-1] == 0:
        ...
        if a[0] == 1:
            male_population += 1
temp_to_csv = pd.DataFrame(temp_to_csv_1)
temp_to_csv.to_csv('data.csv')
print()
print(female_population)
print(male_population)
print(human_population)
#print(human)
print(len(couple))
#print(couple)